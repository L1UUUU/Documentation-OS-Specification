...
A Work's terminal `outcome` is declared in the PRD's front matter when the Work reaches `Completed`. Normative `outcome` values are:
...
An active Work has no `outcome`; the field is set by the Complete stage (DOS-3004). See DOS-3002 for the semantics of each value.
...
INDEX.md SHALL list, under "Active Works" and "Completed Works" sections, each Work's workstream slug, PRD path, and HANDOFF path (and, for Completed Works, its `outcome`), and SHALL list each Issue as `NN-<slug>.md [status] <title>`. Paths MAY include an `active/` or `completed/` segment; INDEX.md is a derived artifact and is regenerated as Work state changes.
